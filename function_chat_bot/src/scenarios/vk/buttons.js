...
export const vkButtons = {
  ...
  // === START ===
  START: [
        [{ text: "ДА, ПОКАЖИ МНЕ ЭТО!", callback: "Start_Choice" }],
      ],
  ...
  Training_Main: (links, user) => {
        const r = [];
        r.push([
          { text: "📖 ВВОДНАЯ БАЗА (10%)", callback: "Theory_Mod1" },
        ]);
        r.push([
          { text: "🔥 ПРАКТИКА (90%)", callback: "Module_1_Strategy" },
        ]);
  ...
  SYSTEM_SETUP: (links, user) => {
        ...
        r.push([
          { text: "📚 ПОВТОРИТЬ ОБУЧЕНИЕ", callback: "Training_Main" },
        ]);
  ...
  TOOLS_MENU: (links, user) => {
        ...
        if (hasMod3) {
          ...
          r.push([
            { text: "🔒 БАЗА ЗНАНИЙ", callback: "LOCKED_KNOWLEDGE" },
          ]);
        }
        ...
        if (isPro) {
          ...
          r.push([
            { text: "🔒 NEUROGEN APPS", callback: "LOCKED_AI_APPS" },
          ]);
        }
  ...
  MY_AI_BOT: (links, user) => {
        ...
        if (user.session?.bot_username) {
          ...
          r.push([
            { text: "✅ ОСТАВИТЬ КАК ЕСТЬ", callback: "MAIN_MENU" },
          ]);
        } else {
          // Если бота нет — кнопка запуска
          r.push([
            { text: "🚀 ПОДКЛЮЧИТЬ БОТА", callback: "SETUP_BOT_START" },
          ]);
          r.push([{ text: "🔙 НАЗАД", callback: "MAIN_MENU" }]);
        }
        return r;
  ...
  MAIN_MENU: (links, user) => {
        ...
        if (isPro) {
          ...
          r.push([
            { text: "💎 PRO-ИНСТРУМЕНТЫ", callback: "Training_Pro_Main" },
          ]);
          ...
          const resumeStep =
            user.saved_state && trainingSteps.includes(user.saved_state)
              ...
              : "Training_Main";
          r.push([
            { text: "🎓 ПРОДОЛЖИТЬ НАСТРОЙКУ", callback: resumeStep },
          ]);
        }
