...
export const telegramButtons = {
  // === START ===
  START: [[{ text: "ДА, ПОКАЖИ МНЕ ЭТО!", callback_data: "Start_Choice" }]],
  ...
  Training_Main: (links, user) => {
    const r = [];
    r.push([{ text: "📖 ВВОДНАЯ БАЗА (10%)", callback_data: "Theory_Mod1" }]);
    r.push([{ text: "🔥 ПРАКТИКА (90%)", callback_data: "Module_1_Strategy" }]);
  ...
  WAIT_SECRET_2: (links, user, info) => [
    [
      {
        text: "📖 ВЕРНУТЬСЯ К СТАТЬЕ",
        url: `${ACADEMY_BASE_URL}/module-2/?bot=${info?.bot_username || "sethubble_biz_bot"}`,
      },
    ],
    [
      {
        text: "🔙 ОТМЕНА (Вернуться в меню)",
        callback_data: "Module_2_Online",
      },
    ],
  ],
  ...
  WAIT_SECRET_3: (links, user, info) => [
    [
      {
        text: "📖 ВЕРНУТЬСЯ К СТАТЬЕ",
        url: `${ACADEMY_BASE_URL}/module-3/?bot=${info?.bot_username || "sethubble_biz_bot"}`,
      },
    ],
    [
      {
  ...
  MULTI_CHANNEL_SELECT: (links, user) => {
    ...
    if (!anyAvailable) {
      r.push([
        { text: "✅ ВСЕ КАНАЛЫ ПОДКЛЮЧЕНЫ", callback_data: "CHANNEL_SKIPPED" },
      ]);
    } else {
  ...
  MULTI_CHANNEL_TG: (links, user) => {
    const r = [];
    r.push([
      { text: "📱 НАСТРОИТЬ TELEGRAM", callback_data: "CHANNEL_SETUP_TG" },
    ]);
    r.push([{ text: "⏭ НАЗАД", callback_data: "MULTI_CHANNEL_SELECT" }]);
    return r;
  },
  ...
  MULTI_CHANNEL_EMAIL: (links, user) => {
    const r = [];
    r.push([
      { text: "📧 НАСТРОИТЬ EMAIL", callback_data: "CHANNEL_SETUP_EMAIL" },
    ]);
    r.push([{ text: "⏭ НАЗАД", callback_data: "MULTI_CHANNEL_SELECT" }]);
    return r;
  },
  ...
  CHANNEL_SETUP_COMPLETE: (links, user) => {
    const r = [];
    r.push([
      { text: "🚀 ПРОДОЛЖИТЬ ОБУЧЕНИЕ", callback_data: "Training_Main" },
    ]);
    r.push([{ text: "🏠 В ГЛАВНОЕ МЕНЮ", callback_data: "MAIN_MENU" }]);
    return r;
  },
  ...
  TOOLS_MENU: (links, user) => {
    ...
    if (hasMod3) {
      ...
      r.push([{ text: "🔒 БАЗА ЗНАНИЙ", callback_data: "LOCKED_KNOWLEDGE" }]);
    }
    ...
    if (isPro) {
      ...
      r.push([{ text: "🔒 NEUROGEN APPS", callback_data: "LOCKED_AI_APPS" }]);
    }
    ...
    if (isPro) {
      ...
      const resumeStep =
        user.saved_state && trainingSteps.includes(user.saved_state)
          ? user.saved_state
          : "Training_Main";
      r.push([{ text: "🎓 ПРОДОЛЖИТЬ НАСТРОЙКУ", callback_data: resumeStep }]);
    }
